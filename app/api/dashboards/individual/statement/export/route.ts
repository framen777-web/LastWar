...
export async function GET(request: Request) {
  ...
  const rankHistory = await getConductorRankHistory(memberId);
  ...
  // there's no need to keep two export shapes in sync with the two on-screen views. Rank
  // is per-row now, so there's no separate header line for it - the last row already
  ...
  const header = ["Week", ...categories.map((c) => c.name), "Conductor Selected", "Net", "Balance", "Rank"];
  sheet.addRow(header);
